...
def test_reject_overlapping_appointment_api(client):
    ...
    ap1 = client.post("/appointments", json={
        "patient_id": pid,
        "doctor_id": did,
    ...
    ap2 = client.post("/appointments", json={
        "patient_id": pid,
        "doctor_id": did,
...
def test_create_appointment_missing_end_returns_400(client, db_session):
    ...
    rp = client.post("/patients", json={"name": "Alice", "email": "alice@example.com", "phone": "1"})
    assert rp.status_code == 200
    pid = rp.json()["id"]
    ...
    assert "appointment_end" in res.json().get("detail", "")


def test_delete_appointment_service_and_not_found(client, db_session):
    db = db_session
    # create patient and doctor
    rp = client.post("/patients", json={"name": "Carol", "email": "carol@example.com", "phone": "2"})
    assert rp.status_code == 200
    pid = rp.json()["id"]
